...
var _canvasExifOrientation = require("canvas-exif-orientation");

var _canvasExifOrientation2 = _interopRequireDefault(_canvasExifOrientation);
...
var Croppie = React.createClass({
	...
	render: function render() {
		...
		return React.createElement(
			...
			React.createElement(
				...
				React.createElement(
					"canvas",
					{ className: "cr-image", ref: "canvasPreview", style: _extends({ zIndex: '-1' }, this.state.previewStyle) },
					" "
				),
				React.createElement("img", { src: "", className: "cr-image", ref: "preview", style: _extends({ zIndex: '1' }, this.state.previewStyle) }),
			...
			prom = new Promise(function (resolve, reject) {
				...
				img.onload = function () {
					...
					_exifJs2.default.getData(img, function () {
						self.orientation = _exifJs2.default.getTag(this, "Orientation");
					});
	...
	_getCanvasResult: function _getCanvasResult(img, data) {
		...
		var orientedCanvas = _canvasExifOrientation2.default.drawImage(img, self.orientation);
		...
		ctx.drawImage(orientedCanvas, left, top, width, height, 0, 0, width, height);
		if (circle) {
			ctx.fillStyle = '#fff';
			ctx.globalCompositeOperation = 'destination-in';
			ctx.beginPath();
